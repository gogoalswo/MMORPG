...
>
> **〔여기서는 — 이펙트는 풀에서 되감아 쓴다〕** ★ (2026-09-23) 끝나면 `queue_free`
> 가 아니라 `finish()` → **`FxPool.give`** 로 숨겨 두고, 다음 시전에 `FxPool.take`
> 로 꺼내 되감는다. 그래서 노드를 짓는 `_build`(한 번)와 되감는 `_start`(매번 —
> 아무것도 새로 만들지 않는다)를 나눈다. 방출기는 `restart()` 로 켠다. 쓸 때마다
> 새로 만들었더니 **스킬마다 히치**가 났고, 다 꺼지면 셰이더까지 버려져 다시
> 컴파일했다 → [skills.md](skills.md) "히치 — 이펙트 풀".
...
> (2026-09-23). 새 이펙트는 `FxWarm._spawn` 과 `FxPool.PREFILL` 에도 한 줄씩 더한다
> → [skills.md](skills.md) "셰이더 미리 굽기" · "히치 — 이펙트 풀".

> **〔여기서는 — 흙먼지〕** (2026-09-23 천붕각, `QuakeFx`) 먼지는 ① **`FxTex.puff`**
> (뭉게뭉게한 뭉치 — `glow` 는 가운데만 진해 물방울무늬가 된다) ② **재질 색은 흰색**
> (입자 색과 재질 색이 두 번 곱해져 모래색이 진흙색이 된다) ③ **`proximity_fade`**
> (큰 판이 바닥에 잘려 곧은 모서리가 보인다) 로 그린다. **밝은 바닥에서는 가산이
> 안 보인다** — 흙 속이 달아오르는 것도 알파로 칠한다.
