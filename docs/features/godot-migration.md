...
아이템 표를 다시 만들기로 해서 **목록이 비어 있다.** 창을 열고 닫는 것, 역할로
창을 가르는 것, **닿는 거리를 `World` 가 다시 재는 것**(`NPC_REACH` 4.5)까지는
돈다 — 아이템 표가 들어오면 `npc_open` 이 채워 보내는 `items` 만 차면 된다.
...
열세 개 다 `pages.yml` 이 배포 전에 돌린다. 하나라도 깨지면 배포까지 가지 않는다.
...
12. **아이템** ← 지금 여기. 다시 만들기로 한 것. 표가 들어오면 상점·대장간
    목록과 스킬 화면 옆의 가방이 채워진다
13. **서버** — 고도 헤드리스. `Transport` 에 구현을 하나 더 끼운다
