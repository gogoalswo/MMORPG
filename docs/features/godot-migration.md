...
### 모델 텍스처는 손실(WebP 0.9)로 굽는다 ★ (2026-09-26)

"로딩이 너무 오래 걸린다" 를 재 보니 **`index.pck` 가 49MB** 였고 절반(24.9MB)이 모델
텍스처였다. 고도는 GLB 에서 떼어낸 JPG 를 기본값 **무손실**로 굽는데, 원본이 이미
손실(JPG)이라 무손실로 보존해 봐야 JPG 흠집까지 지킬 뿐이다 — 90KB JPG 가 **675KB** 가 됐다.

그래서 `project.godot` 의 `[importer_defaults]` 로 텍스처 기본값을 **손실 0.9** 로 바꿨다.
GLB 가 떼어낸 텍스처도 이 기본값을 따른다 (시험해 봤다). **모델 텍스처 24.9MB → 3.85MB,
pck 49MB → 약 28MB.**

| 품질 | 한 장(오우거 5) | 원본 JPG 대비 PSNR |
|---|---|---|
| 0.7 | 52KB | 31.0dB |
| 0.8 | 67KB | 32.7dB |
| **0.9 (쓰는 값)** | 100KB | 35.2dB — 격투가 39.8dB |

- **UI 아이콘은 무손실로 둔다** — 얇은 금선이 뭉개진다. `sync-godot-assets.mjs` 가
  `ICONS`·`UI` 를 복사할 때 **고도가 임포트하기 전에** `compress/mode=0` 만 적은
  `.import` 를 써 둔다(`LOSSLESS_IMPORT`). 고도는 있는 `.import` 를 따르고 나머지 칸을 채운다.
- `.import` 는 커밋하지 않고 CI 는 매번 새로 임포트하므로 CI 빌드에는 바로 먹는다.
  **로컬에 예전 `.import`(무손실)가 남아 있으면 로컬만 그대로다** — 익스포트는 CI 에서만
  하므로 상관없다.
- `model_test.gd` 의 `_case_texture_lossy` 가 기본값이 손실인지, 아이콘이 무손실인지 본다.
- VRAM 압축(ETC2·S3TC)은 쓰지 않았다 — 웹은 두 포맷을 다 실어야 해서 커진다 (위 표의 Basis 시도 참고).
...
- **떨어지는 장비는 잡은 사람이 쓸 수 있는 것만** 고른다. 못 쓰는 무기가 가방을
  채우면 정리하는 게 일이 된다
- **등급은 사냥터가 정한다** (2026-09-21). 사냥터 F 에서는 **그 사냥터의 등급과
  아래 하나**만 나온다 — Lv1 몬스터가 최고 등급을 떨굴 수 없다. 표는
  `items.json` 의 `dropGrades` 에 있고 고도는 읽기만 한다 (`Items.drop_grades`).
  `maxDropGrade` 는 제작을 걷으며 잠깐 10 이었다가 **7로 되돌렸다** —
  사냥터 상한이 있으면 "길이 끊긴다"는 이유가 사라진다 → [items.md](items.md)
...
`user://save.json` 에 존·자리·레벨·경험치·체력·골드·사망·스킬·가방·장비와 **한 번만 받는 것의 표시(`granted`)** 만 남긴다. 화면이 만들어
...
| 창 | 규칙 |
|---|---|
| 상점 | **자기 직업 무기만.** 방어구·장신구는 사냥으로만 줍는다. 팔 때 `price × 0.4 × 등급배율` — 등급이 높으면 더 쳐준다 |
| 강화 | **공짜다.** 성공 / 파괴 둘뿐 — 설계(stat-balance.md 4장)를 그대로 쓴다 |

**새로 만들기·등급 올리기는 2026-09-20 에 없앴다** — 제작 자체를 걷었다
→ [items.md](items.md) · [npc-town.md](npc-town.md).
...
| 상점·대장간 (사고팔기·강화) | `... tests/shop_test.gd` |
...
**`npm run test:godot` 은 멈추지 않는다** ★ (2026-09-24, `scripts/godot-test.sh`).
- **스크립트가 클래스 캐시(`godot/.godot`)보다 새로우면 먼저 `--import` 한다.** 다른 세션이
  `class_name` 을 새로 넣은 것(`DragScroll`)을 받았더니 로컬 캐시가 그 이름을 몰라 `game.gd` 가
  파스 에러를 냈다. CI 는 늘 임포트부터 해서 안 걸렸다.
- **테스트 하나에 120초 제한** (`GODOT_TEST_TIMEOUT` 으로 바꾼다). 스크립트 에러가 나면
  `quit()` 에 못 닿아 고도가 끝나지 않고 기다린다 — 위 파스 에러로 camera·chat_log 가 20분 넘게
  멈춰 전체가 안 끝났다. 넘기면 "N초 안에 안 끝났다" 로 실패 처리하고 다음으로 간다.
...
13. ~~**상점·대장간** — 사고팔기·강화~~ 끝 (2026-09-17, 제작은 2026-09-20 에 걷었다)
...
| `index.pck` (게임) | 약 28MB | 약 28MB (텍스처·메시는 gzip 이 거의 안 먹는다) |

**오우거 2~5 를 넣으면서 10.3MB → 20.6MB**(2026-09-23), **NPC 7 벌 · 장비 `gear_g1~7`
7 벌을 넣으면서 49MB** 까지 늘었다. 모델이 21 벌이고 한 벌에 2MB 쯤이다 (메시 약 1MB ·
텍스처). **텍스처를 손실로 구워 약 28MB 로 줄였다** (위 "모델 텍스처는 손실로 굽는다").
나머지 19MB 는 모델 메시(`.scn`, 한 벌에 정점 2만 3천)다.

**오우거 동작을 한 벌만 싣는 건 효과가 없다** (2026-09-26 에 재 봤다). 동작은 한 벌에
0.18MB, 다섯 벌 합쳐 0.9MB 뿐이다. 게다가 다섯 벌의 동작은 **이름·길이만 같고 값이 다르다** —
바르코가 몸마다 따로 입혀서, 뼈 길이가 최대 121%·기본 자세가 최대 24° 다르고, 오우거 1 의
동작을 전역 기준으로 옮겨 봐도 팔뚝·다리가 최대 18° 어긋난다
→ [characters-and-animation.md](characters-and-animation.md) 의 오우거 절.
