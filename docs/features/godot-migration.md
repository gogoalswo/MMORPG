...
### 땅을 누르고 있으면 계속 따라 걷는다 ★ (2026-09-26)

"지면을 클릭하고 있으면 해당 위치로 이동하게 만들어. 지금은 누른 지점만 움직여" 로
고쳤다. 예전에는 누르는 순간 한 번만 `_target` 을 잡아서, 누른 채 끌어도 처음 점으로만
갔다.

- 땅을 누르면 `_holding` 을 켜고 화면 점을 `_hold_at` 에 둔다. `_send_input` 이 **매 프레임**
  그 화면 점 아래 바닥(`_ground_point`)으로 `_target` 을 다시 잡는다(`_set_target`).
  캐릭터가 걸으면 카메라가 따라와서 **손가락을 가만히 둬도 목표가 앞으로 밀린다** —
  누르고 있는 동안 계속 걷는다.
- 끌기·떼기는 `_input` 이 받는다. `_unhandled_input` 은 UI 가 먹은 이벤트를 못 받아서,
  창 위에서 손을 떼면 계속 걸어가 버린다.
- 몬스터·NPC·차원문을 누른 것은 누르고 있기가 아니다 — 땅을 누른 것만 켠다.
  새로 누르거나 죽으면 끈다.
- 터치는 기본 설정이 마우스로 바꿔 주므로 폰도 같은 길을 탄다.
- 확인은 `touch_test.gd` — 누른 채 끌면 목표가 따라오고, 떼면 멈춘다.
...
| 눌러서 걸어가기 · 누른 채 따라 걷기 | `... tests/touch_test.gd` |
