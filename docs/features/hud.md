...
물약 칸은 2026-09-26 에 퀵슬롯 바로 옆에 더했다 → 아래 "물약 칸".
...
| ″ | `_refresh_potion` · `_build_potion_panel` · `_potion_step` — **물약 칸**과 설정 창. 아래 "물약 칸" |
...
| `godot/tests/ui_test.gd` | `_case_potion` — 퀵슬롯 옆 자리 · 설정 창 +/− · 눌러서 마시고 쿨타임이 돈다 |
...
- **자리** — 퀵슬롯 4칸 바로 오른쪽, 자동사냥 칸(`AUTO_GAP` 만큼 띈다) 왼쪽. 칸은 퀵슬롯과
  같은 `_make_skill_cell(QUICK_CELL, …)` 이라 쿨타임 어둠·바늘·남은 초·다 돌면 번쩍이 그대로다.
...
- **칸 오른쪽 위 "설정"** → 가운데에 설정 창. −/+ 로 기준을 **10%p 씩, 0 ~ 90%**. 0 이 "자동 끔".
  칸 아래 배지가 지금 기준(`HP 70%` / `자동 끔`)을 적는다. 값은 `potionPct` 요청으로 보내고
  **판정(`set_potion_pct`)이 잘라서** 스냅샷으로 돌아온 값만 그린다 (자동사냥 칸과 같은 규칙).
- **수치**는 shared `combat.ts` 의 `POTION_*` → `combat.json` 의 `potion*`:
  쿨타임 10초 · 한 병에 **최대 HP 10%** · 처음 기준 **70%** · 폭 10 · 상한 90.
  (처음에는 30% · 50% 였다 — 같은 날 "초기값 70%로 하고 물약 한번 마시면 10% 차도록" 요청으로 바꿨다.)
...
- 기준은 **저장에 남는다** (`save.gd` 의 `potion_pct`, 없던 칸이라 옛 저장은 처음 값 70).
  쿨타임은 존을 옮겨도 이어진다 (`join` 의 `kept`) — 차원문을 오가며 연달아 못 마신다.
- 아이콘(`ui_icon_potion`)이 아직 없어 **글자 "물약"** 으로 나온다. 그림을 받으면 이름만 등록하면 된다
  (`sync-godot-assets.mjs` 의 `ICONS`).
