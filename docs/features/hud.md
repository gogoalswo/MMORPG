...
| `godot/game/loot_log.gd` | **획득 알림** (`LootLog`) — 왼쪽에 경험치·장비 획득을 한 줄씩 쌓는다. 아래 "획득 알림" |
...
| `godot/tests/loot_log_test.gd` | 진짜로 잡으면 경험치 줄이 뜨나 · 장비 줄 · 6줄 상한 · 단추와 안 겹침 · 4초 뒤 사라짐 |
...
오른쪽 위 메뉴 세 번째 **"설계"** 단추. [stat-balance.md](stat-balance.md) 9장 5번이
...
몬스터를 잡으면 **화면 왼쪽**에 한 줄씩 쌓인다 — `경험치 +12`, `장비 획득 희귀 무기`.

```
초원  골드 5  몬스터 …          ← 위 글자줄 (24,24)
<마지막에 일어난 일>
┌────────────────────┐          ← LOOT_LOG_TOP 96
│ 장비 획득  희귀 무기 │  (이름은 등급 색)
│ 경험치  +12         │  (금빛)
└────────────────────┘          …6줄까지. 꽉 차도 아래 끝 324 — 테스트 단추(474~) 위
```

- **왜 왼쪽인가.** 처음엔 쓰러진 자리에서 3D 글자로 띄웠는데 **"데미지랑 같이
  뜨니까 안 보여. 왼쪽에 ui 를 만들어서 넣어"** 라는 요청을 받았다. 가운데는
  전투가 쓴다 → [hit-effects.md](hit-effects.md)
- **결** — `ui-art-style.md` 의 어두운 판(`#191a19`, 78% 불투명) + 1px 금테
  (`#b9a46c`). 한 줄짜리 띠라 조각 그림 없이 `StyleBoxFlat` 으로 그린다.
  머리말은 눌린 상아빛, 값이 먼저 읽히게 값만 색을 준다
  (경험치 `#e8c14a` · 장비는 가방 칸과 같은 `_grade_tint`).
- **이름에 등급이 이미 들어 있다** ("희귀 무기") — 등급 글자를 따로 붙이지 않는다.
- 새 줄이 **맨 아래**, `MAX_LINES` 6 줄을 넘으면 오래된 것부터 민다.
  한 줄은 `LIFE` 4초(끝 0.6초 흐려짐), 들어올 때 왼쪽에서 40px 밀려 들어온다.
- **골드는 안 띄운다** — 요청이 경험치와 장비였다. 골드는 위 글자줄에 그대로 있다.
  가방이 꽉 차 장비를 못 받으면 `loot` 에 `item` 이 없어서 줄도 안 뜬다.
- 누르는 것이 아니다 — 입력을 전부 흘린다(`MOUSE_FILTER_IGNORE`).
- ★ **줄 노드는 지우지 않고 숨겨 두었다 되쓴다** (`_spare`, 여섯 줄을 미리 짓는다).
  한 줄이 컨테이너 둘 · 글자 둘 · 테마 덮어쓰기라 만드는 데 1.6~2.2ms 였고, 범위
  스킬로 무리를 잡으면 한 프레임에 몰려 **스킬 히치**가 됐다 (2026-09-23).
  꽉 차면 가장 오래된 줄을 맨 아래로 옮겨 글자만 바꾼다 → [skills.md](skills.md) "히치".

## 확인

- `npm run test:godot -- ui` — 자리·숫자·안 겹침·고리가 도는지까지 수치로 본다.
- `npm run test:godot -- loot_log` — 획득 알림.
- `npm run shot:godot -- kill` — 한 대 잡고 장비 두 개를 넣은 화면
  (`logs/shot_kill_*.png`, 알림은 왼쪽 끝이라 `shot_sheet` 에서는 잘린다).
