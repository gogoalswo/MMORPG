...
### 등급 (1~10)
- **드롭이 10등급까지 전부 낸다**(`MAX_DROP_GRADE = GRADE_MAX`). 2026-09-20 에 제작을
  걷으면서 8~10 으로 가는 길이 드롭밖에 안 남았다 — 7 에서 끊으면 도달 불가가 된다.
- 드롭 등급 가중치: `2^(10-g)` — 한 등급 오를 때마다 절반.
...
따라온 것 둘:
- `MAX_DROP_GRADE` 가 7 → 10. 드롭이 유일한 길이 됐으니 8~10 도 떨어져야 한다.
