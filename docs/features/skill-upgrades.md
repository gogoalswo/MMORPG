...
- **1번 파쇄** (`followMs` 1100 · `followPower` 0.8) — **뒤따르는 한 대**: 첫 대로 맞은 놈에게
  ...
  - 이펙트: 기둥이 0.28초에 땅으로 꺼지던 것을 **0.05초에 빠지게** 하고(`BREAK_SINK`), 그 순간
    기둥 허리에서 **조각 130개**(길쭉한 70 + 뭉툭한 60, 초속 4~9)가 사방으로 터진다 — 원래 꺼질
    때 튀던 조각(60개)에 더해진다. 섬광과 번쩍임 빛이 부서지는 순간 한 번 더 켜진다.
...
- **강화를 더할 때** — `SKILL_UPGRADES` 에 한 줄(`exp` 필수, 효과는 `stunMs` · `rangeMul` · `arcAdd` · `extraHits` · `targetsAdd` · `zoneMs`·`zoneTickMs`·`zonePower` · `followMs`·`followPower` · `stunLook` 중에서) → `npm run export:godot`.
