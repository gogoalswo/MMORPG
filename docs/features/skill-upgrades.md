...
| 빙주각 `frost_pillar` | 1번 파쇄 `shatter` | 1000 | 첫 대로 맞은 놈에게 **1.1초 뒤 한 대의 80%** 를 한 번 더 | 기둥이 땅으로 꺼지는 대신 **그 자리에서 부서져** 조각이 크게 터지고, 섬광·빛이 한 번 더 | 붙었다 (1차 시안) |
...
- **1번 파쇄** (`followMs` 1100 · `followPower` 0.8) — **뒤따르는 한 대**: 첫 대로 맞은 놈에게
  1.1초 뒤 `한 대 × 0.8` 을 한 번 더 넣는다. 연타 예약(`_combos`)을 그대로 쓰므로 그 사이 죽은
  놈은 건너뛴다. 연타(`extraHits`)와 달리 **간격·배율이 강화마다 따로**다.
