...
| 천붕각 `sky_breaker` | 2번 균열 지대 `zone` | 1000 | 시전한 자리에 **3초간 0.5초마다 공격력 40%** (여섯 번) | 틈이 용암빛으로 남고 **틱마다 틈에서 용암이 분수처럼 솟는다**, 틱 사이 방울 | 붙었다 (2차 시안) |
...
| `godot/game/quake_fx.gd` `WIDE` · `ZONE_*` · `COLOR_LAVA` · `_lava_tint` · `_ground` | 강화한 천붕각 — 틈의 용암빛·빛, 진폭이면 먼지 충격파·금을 끈다 |
| `godot/game/quake_parts.gd` `QuakeParts.Tornado` · `QuakeParts.Lava` | ★ **모래 토네이도**(띠 메시 한 번 깔고 셰이더가 돌린다 + 먼지 덩이)와 **틈에서 솟는 용암**(분수·방울·연기) |
| `godot/tests/skill_test.gd` `_case_quake_up` | 진폭 9m·15마리, 지대 틱 여섯 번·한 틱 = 공격력 40%·지대 밖으로 나간 놈은 안 맞음 |
| `godot/tests/quake_fx_test.gd` `_case_upgrades` | 진폭 토네이도가 9m 까지·먼지 충격파·금 없음, 지대 3초 붉게·틱마다 맥동·용암 분수 여섯 번·방울은 지대 동안만, 판정 표와 시간이 같나 |
...
- **균열 지대 이펙트** (2차 시안, 2026-09-24) — 1차(틈이 붉게 맥동)는 **"갈라진 틈새에서
  지속적으로 용암이 터져 나오는 이펙트로"** 로 바뀌었다. 틈의 용암빛·맥동·빛은 그대로 두고
  **틈에서 솟는 용암**을 더했다 (`QuakeParts.Lava`):
  - **틱마다(0.5 · … · 3.0초) 분수** — 본줄기 금 여덟의 60% 지점에서 굵은 덩이 110개
    (0.5m, 초속 6.5~10, 중력 −20)가 솟았다 떨어진다. 노랑 → 주황 → 검붉게 식고 작아진다.
    검은 연기가 조금 따라 오른다. 1차(0.26m · 64개 · 흰빛에서 시작)는 **흩어진 옅은 불똥**이었다.
  - **틱 사이 방울** — 모든 금 점에서 작은 덩이가 뽀글뽀글 튄다 (0.3초 ~ 지대 끝).
  - 방출기는 금과 같은 쪽으로 돈 노드 아래에 있고, 진폭이면 솟는 자리도 1.5배 벌어진다.
  - 용암도 **알파 혼합**이다 — 밝은 바닥에서 가산은 안 보인다.
- **균열 지대 — 틈의 용암빛** — 금의 달아오른 심(`_glow`)이 평소처럼 0.8초에 식지 않고 **3초 동안
...
- **찍어서 봤다** — 토네이도는 0.12 · 0.3 · 0.5 · 0.75초 네 장과 발밑 확대, 용암은 0.6 · 0.8 ·
  1.1 · 2.3초 네 장. 새 강화 이펙트 셰이더·방출기는 `FxWarm` 이 한 번 띄워 굽는다.
