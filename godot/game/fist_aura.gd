...
## 무기(건틀릿)를 낀 주먹에 감도는 기운(오로라). **강화 +5 부터 나온다** —
## +4 까지는 없다. **색은 강화 단계가 정한다**: +5 초록 · +6 파랑 · +7 빨강 ·
## +8 하양 · +9 황금 (2026-09-25 요청: "등급에 따라서 오로라 색상을 바꾸지 말고
...
## 오로라가 나오는 첫 강화 단계
## 처음엔 +6 부터(+6 초록 … +10 황금)였는데 최대 강화가 +9 라 황금이 안 나와서
## **+5 부터로 당겼다** (2026-09-25, 사용자가 "최대는 +9 로 두고 +5 부터" 를 골랐다)
const FIRST := 5
## 강화 단계별 색 (+5 ~ +9 = 최대 강화).
## **하양만 알파를 낮춘다** — 가산이라 R·G·B 가 다 차 있는 하양은 겹치는 곳마다
## 하얗게 타서 태초가 주먹 둘레 전체가 흰 덩어리로 보였다 (2026-09-25 찍어 봄)
const COLORS := {
	5: Color("#46ff78"),
	6: Color("#3d8cff"),
	7: Color("#ff3838"),
