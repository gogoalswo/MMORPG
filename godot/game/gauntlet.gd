...
## 오른손 가운데와 반 크기 (손·손가락 뼈에 묶인 정점 640개의 상자). 왼손은 x 를 뒤집는다.
## 2026-09-26 — 몸을 **주먹 쥔 A 자세 바르코 모델**로 바꿨다 (조각된 주먹, 정점 640개). 잴 때는
## `node scripts/measure-bones.mjs <glb> RightHand --posed` (손가락 뼈 정점을 그 손 몫으로 센다).
## 편 손 몸: (-0.012, 0.063, -0.007) ± (0.046, 0.063, 0.028)
const FIST_CENTER := Vector3(0.016, 0.043, 0.008)
const FIST_HALF := Vector3(0.041, 0.043, 0.045)
