...
func _build_sparks(tint: Color) -> void:
	...
	chip.size = Vector3(0.09, 0.09, 0.09)
	var mat := _glow(tint)
	for i in SPARKS:
		...
		add_child(node)
		var angle := randf() * TAU
		var out := randf_range(1.8, 3.4)
		_sparks.append({
			"node": node,
			"vel": Vector3(cos(angle) * out, randf_range(2.0, 3.4), sin(angle) * out),
		})
...
func _build_number(amount: int, tint: Color, heal: bool, crit: bool, font: Font) -> void:
	...
	_number.pixel_size = 0.009 if crit else 0.006
	_number.modulate = tint
	_number.outline_size = 12
	...
	_number.position = Vector3(randf_range(-0.25, 0.25), 0.2, 0.0)
	add_child(_number)
...
func flash_body(body: Node3D) -> void:
	...
	mat.albedo_color = Color(1.0, 0.3, 0.25, 0.5)
