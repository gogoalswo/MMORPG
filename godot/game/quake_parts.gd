...
class Mud:
	...
	## **촘촘한 회오리** (2026-09-24 요청 — 사용자가 캡처를 주며 "이런식으로 촘촘하게 회오리").
	## 넷 · 반 바퀴였을 때는 성긴 바람개비였다. 여덟 줄이 한 바퀴 남짓 감겨 들어야 그림처럼
	## 촘촘하다. (웅덩이가 띠를 덮던 때에는 이렇게 감으면 동심원 고랑이 됐지만, 웅덩이를 걷고
	## 띠 속을 채운 지금은 나선으로 읽힌다)
	const ARMS := 8
	## 반지름이 바깥 1 → 0.1 로 줄 때까지 감기는 바퀴 수 — **로그 나선**이라 어디서나 같은
	## 각(≈22°)으로 휘어 든다. 반지름을 고르게 줄이면(1 − u) 바깥쪽이 거의 원(8°)이라
	## 여덟 줄이 **동심원**으로 보였다 (캡처)
	const TURNS := 0.92
	const POINTS := 96
	## 띠 폭(m, 바깥 끝 기준 — 가운데로 갈수록 가늘어진다). 줄이 늘어 좁혔다 — 넓으면 이웃과
	## 붙어 띠 사이 어두운 틈이 없어진다
	const ARM_WIDTH := 1.8
	...
	static func arm_mesh() -> ArrayMesh:
		...
		for b in ARMS:
			...
			for p in POINTS + 1:
				...
				# 로그 나선 r = e^(−kθ) — 바깥(1)에서 0.1 까지 같은 각으로 감겨 든다
				var rf := pow(0.1, u)
				for side in [0.0, 1.0]:
