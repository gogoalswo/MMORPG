...
func _case_game() -> void:
	...
	# 새 캐릭터는 시작 장비(`World.STARTER_SLOTS`, 1등급)만 입고 시작한다
	for slot in Armor.SLOTS:
		var start := 1 if slot in World.STARTER_SLOTS else 0
		if rig.gear_grade(slot) != start:
			_fail("처음 %s 가 %d등급 — %d등급이어야 한다" % [slot, rig.gear_grade(slot), start])
	for grade in [2, 6]:
