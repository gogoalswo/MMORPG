 ...
 *
 * **아직 게임에 붙이지 않았다.** 설계 문서 9장이 "아이템은 마지막" 이라고 적어 둔
 * 순서를 지키는 것이다 — 스탯(`stats.gd`)이 먼저 들어가야 이 %가 곱할 기본값이 생긴다.
 * 붙이는 날 `items.ts` 를 이 표로 갈아끼우면 된다.
 */
import { JOB_IDS, type JobId } from './character.ts';
import { EQUIP_SLOTS, SLOT_CODE, type EquipSlot } from './items.ts';
