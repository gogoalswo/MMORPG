...
test('등급 이름은 일반 → 태초 일곱 개다', () => {
  // 2026-09-21 지시. 어느 사냥터 물건인지가 아니라 **희소도**가 등급 이름이다
  assert.deepEqual(
    [1, 2, 3, 4, 5, 6, 7].map((g) => gradeName(g)),
    ['일반', '고급', '희귀', '영웅', '전설', '초월', '태초']
  );
});

test('장비 이름은 등급마다 재질이 오른다 (건틀릿)', () => {
  // 2026-09-23 사용자가 고른 안 A. 등급은 이름 색이 알려 주므로 이름에 등급 글자가 없다
  assert.deepEqual(
    Object.values(ITEMS)
      .filter((i) => i.slot === 'weapon')
      .sort((a, b) => a.grade - b.grade)
      .map((i) => i.name),
    ['가죽 건틀릿', '강철 건틀릿', '흑철 건틀릿', '투사의 건틀릿', '용린 건틀릿', '성운 건틀릿', '창세의 건틀릿']
  );
  assert.equal(ITEMS['g5_r']?.name, '용안 반지');
});
...
test('등급이 곧 성능이다 — 무기는 등비 ×2.434 로 오른다', () => {
  ...
  // 공격력 축은 2026-09-24 에 ×1.7037 → ×2.434 (영웅 무기 = 일반의 피해 3배, `gear.ts` `gradeRatio`)
  const ratio = 2.434;
