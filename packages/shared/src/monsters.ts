 ...
 *
 * 그 전에는 **이 줄에서 그 자리에 역산**했다(레벨 + 그 레벨의 기준 장비). 그래서
 * 장비 지분을 손대면 몬스터가 소리 없이 따라 움직였다 — 2026-09-21 에 장신구
 * 배분을 바꾸자 Lv200 HP 가 -30% 가 됐다. 지금 표는 그때 얼린 값이고, 바꾸려면
 * `monsterTable.ts` 를 손으로 고친다 (전부 다시 뽑으려면 `npm run freeze:monsters`).
 *
 * 더 옛날(2026-09-20 이전)에는 `HP = 20L + 40` 같은 손으로 고른 식이었다.
 *
