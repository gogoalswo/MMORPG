 ...
 * 된다. 각 스킬의 `cooldown` 데이터는 건드리지 않는다.
 * 2026-09-24 에 쿨타임을 할퀴기 1초 · 나머지 10초로 정하면서 false 로 껐다
 * (켜 두면 정한 값이 안 보인다). 고도에서는 화면 왼쪽 아래 단추로 다시 켤 수 있다.
 * 켜져 있어도 서버 자동 시전은 `AUTO_SKILL_TEST_GAP` 마다 한 번만 쏜다 (ZoneRoom 참고).
 */
export const SKILL_COOLDOWN_OFF = false;
