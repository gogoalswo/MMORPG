 ...
 *
 * 모든 존을 size 62 (이동 가능 영역 54) 로 맞췄다. 원래 92 — 한 화면에 보이는 지면
 ...
 * 떨어져 있고(반경 2.6), 무리(±14, 반경 13) 가장자리에서 4m 넘게 떨어져 있다.
 * 원래 9 였는데 맵을 2/3 로 줄이며 무리가 ±14 로 당겨져 9 에서는 동쪽 두 무리
 * 가장자리와 문이 겹친다 — 4.3 안쪽이라야 `zones.test.ts` 의 "무리 반경 + 4" 를 지킨다.
 ...
 *
 * 12시를 -z(북)로 놓고 시계방향으로 잰다. 원래 7시·24유닛이었는데 맵을 2/3 로
 * 줄이며 무리가 네 귀퉁이(±14, 반경 13)를 거의 다 덮어, 무리 원 밖으로 남는 자리가
 * 두 무리 사이 축 위뿐이다. 남쪽 두 무리 원(반경 13)과 보스 원(반경 3)이 안 닿고
 * (중심 간 16.6) 벽(27) 안에 드는 거리가 23 이다.
 */
 ...
 * 네 귀퉁이 간격이 28 이라 반경 13 끼리는 2m 떨어져 겨우 안 닿는다.
 */
const PACK_COUNT = 50;
 ...
 *
 * ±28 → ±14 로 당겼다 (2026-09-23, 맵 2/3). 반경 13 · 50마리는 그대로 두기로 했다
 * (사용자 선택) — 그래서 무리 바깥 끝이 14 + 13 = 27 로 **이동 가능 영역 끝(±27)에
 * 딱 닿는다.** 더 바깥으로는 못 민다. 그 대가로 전에 지키던 두 가지를 놓았다.
 *
 * 1. 도착 지점(0,0)이 무리의 **반경 + 인식 범위** 밖이 아니다 — 중심까지 19.8 이라
 *    인식 범위(최대 16)가 닿는다. 사냥터에 들어서면 몬스터가 달려온다.
 * 2. 자동 사냥 반경(`world.gd` 의 `HUNT_RADIUS` 27)이 옆 무리까지 닿는다 — 무리 간격이
 *    28 이라 "무리 하나는 통째로(≥26) · 옆 무리는 안" 을 동시에 만족하는 반경이 없다.
 *
