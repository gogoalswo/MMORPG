...
- 발은 어깨너비, 무릎을 살짝 굽히고, 팔은 **편 손으로 편하게 내린다** (사용자가 준 선 자세 그림대로).
...
def stance(poser, breath):
    ...
    for side, sign, bones, key in (("Left", 1, fm.ARM_L, "lh"), ("Right", -1, fm.ARM_R, "rh")):
        ...
        # 참고 그림(2026-09-26, 사용자가 준 선 자세): 윗팔이 몸통에서 10~15° 벌어지고 팔꿈치가 아주
        # 살짝 굽어 아래팔이 조금 앞으로, 손은 허벅지 바깥 사타구니 높이에서 손바닥이 허벅지를 본다.
        # 그래서 손은 팔 길이의 96.5% 아래 · 바깥 0.095 · 앞 0.035, 팔꿈치(폴)는 바깥 뒤를 본다.
        # (93% 에 팔꿈치를 뒤로 뺐더니 "팔이 너무 뒤로 꺾였다". 들숨에 팔이 조금 더 벌어진다)
        hand = shoulder + Vector((sign * (0.095 + 0.008 * breath), -0.035, -reach * 0.965))
        spec[key] = tuple(hand)
        spec[key + "Pole"] = (sign * 0.7, 0.7, 0.0)
    return spec
