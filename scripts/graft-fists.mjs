...
// 강체 + 균일 배율 역행렬 (역바인드는 그렇다)
function invert(m) {
  const s2 = m[0] * m[0] + m[1] * m[1] + m[2] * m[2];
  const r = [m[0], m[4], m[8], m[1], m[5], m[9], m[2], m[6], m[10]].map((v) => v / s2);
  const t = [0, 1, 2].map((row) => -(r[row * 3] * m[12] + r[row * 3 + 1] * m[13] + r[row * 3 + 2] * m[14]));
...
for (const side of ['Right', 'Left']) {
  ...
  for (let t = 0; t < fIdx.length; t += 3) {
    ...
    if (!tri.every((v) => sideOf[v] === side)) continue;
    for (const v of tri) {
      if (!remap.has(v)) {
